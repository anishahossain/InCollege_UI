...
*>    Each account record consists of a username and password
      ...
      01  ws-user-input    pic x(500) value spaces.
...
*>    A - profile menu logic
    ...
    *> Blank line before message (JSON-friendly)
    move " " to ws-message
    perform display-info
    ...
    *> Message body (smart wrapping)
    ...
    perform until ws-desc-idx > ws-desc-len
        ...
        if ws-remaining <= 200
            ...
            *> Avoid splitting in the middle of a word
            ...
            perform varying ws-j from ws-j by -1
                until ws-j < ws-desc-idx
                   or ws-rem-text(ws-j:1) = " "
                continue
            end-perform
        ...
        *> Skip any leading spaces at the next start
        perform until ws-desc-idx > ws-desc-len
    ...
    *> Reset accumulators
    ...
    exit paragraph.
      ...
      display-title.
    move "TITLE" to ws-output-tag
    move ws-message to ws-output-text
    perform send-json-output
    exit paragraph.



      display-option.
    move "OPTION" to ws-output-tag
    move ws-message to ws-output-text
    perform send-json-output
    exit paragraph.
...
display-prompt.
    ...
    exit paragraph.
        ...
        display-error.
            move "ERROR" to ws-output-tag
            move ws-message to ws-output-text
            perform send-json-output
            exit paragraph.

      display-success.
    move "SUCCESS" to ws-output-tag
    move ws-message to ws-output-text
    perform send-json-output
    exit paragraph.
